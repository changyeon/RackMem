...
static blk_status_t __null_close_zone(
...
static blk_status_t null_zone_write(
        struct nullb_cmd *cmd, sector_t sector, unsigned int nr_sectors,
        bool append)
{
    ...
    if (zone->cond == BLK_ZONE_COND_CLOSED || zone->cond == BLK_ZONE_COND_EMPTY) {
...
static blk_status_t null_open_zone(
        struct nullb_device *dev, struct nullb_zone *zone)
{
...
static blk_status_t null_close_zone(
        struct nullb_device *dev, struct nullb_zone *zone)
{
    blk_status_t ret;
...
static blk_status_t null_finish_zone(
        struct nullb_device *dev, struct nullb_zone *zone)
{
    blk_status_t ret = BLK_STS_OK;
...
static blk_status_t null_reset_zone(
        struct nullb_device *dev, struct nullb_zone *zone)
{
    if (zone->type == BLK_ZONE_TYPE_CONVENTIONAL)
        return BLK_STS_IOERR;
...
static blk_status_t null_zone_mgmt(
